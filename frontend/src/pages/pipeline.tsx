...
export default function PipelinePage() {
  ...
  const fetchPipelineData = async () => {
    ...
    try {
      ...
      const transformedLayers = data.layers.map(
      ...
      setPipelineStatus({
        available:
          data.pipelineAvailable !== undefined ? data.pipelineAvailable : true,
        mode: data.mode || "full",
      });
    } catch (err: any) {
      console.error("Failed to fetch pipeline info:", err);
      setError(
        "Failed to load pipeline information. Ensure backend is running on http://localhost:8000",
      );
    } finally {
      setLoading(false);
    }
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="font-semibold text-dark-text">
                  Pipeline Status:{" "}
                  {pipelineStatus.available ? "Active" : "Limited"}
                </span>
          ...
          >
            ...
            <div className="glass-card p-4 text-center">
              <div className="text-3xl font-bold text-verified mb-1">
                {layers.filter((l) => l.status === "active").length}
              </div>
                ...
                {activeLayer ? (
                  ...
                  >
                    ...
                    <p className="text-sm text-dark-muted">
                      Click on any layer in the pipeline to view detailed
            ...
            <div className="grid md:grid-cols-3 gap-6 text-sm">
              <div>
                ...
                <p className="text-dark-muted">
                  Source documents and LLM output are ingested, parsed, and
                  preprocessed. Claims are extracted and structured for
                  analysis.
                </p>
              </div>
              <div>
                ...
                <p className="text-dark-muted">
                  Each claim is matched against source evidence using semantic
                  search. NLI models determine if claims are supported,
              ...
              <div>
                ...
                <p className="text-dark-muted">
                  Results are scored, corrections generated, and explanations
                  synthesized. The final report includes color-coded annotations
                  and citations.
                </p>
              </div>
            </div>
        ...
        >
          ...
          >
            <div className="flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-xl bg-gradient-to-br ${layer.color} flex items-center justify-center shrink-0`}
              >
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {layers.map((layer, idx) => (
        ...
        >
          <div
            className={`w-10 h-10 rounded-lg bg-gradient-to-br ${layer.color} flex items-center justify-center mb-3`}
          >
        ...
        <div>
          ...
          <ul className="space-y-1.5">
            {layer.details.map((detail, idx) => (
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${layer.bgColor} mt-1.5 shrink-0`}
                />
                {detail}
              </li>
            ))}
