...
export default function BenchmarksPage() {
  ...
  const [selectedTab, setSelectedTab] = useState<'overview' | 'samples' | 'history'>('overview');
  ...
  const runBenchmark = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to run benchmark. Is the backend running?');
    } finally {
      setIsRunning(false);
    }
  };
    ...
    >
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-dark-text">HaluEval Dataset</h2>
          ...
          {hasRun && !isRunning && !error && (
            ...
            >
              ...
              <span className="text-green-300">
                Benchmark completed! Processed {results.processedSamples} samples using {results.pipelineUsed} pipeline.
              </span>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {!hasRun && !isRunning && (
        ...
        >
          ...
          <h3 className="text-lg font-semibold text-dark-text mb-2">No Benchmark Results Yet</h3>
          <p className="text-dark-muted mb-4">
            Click "Run Benchmark" to test the Hallucination Hunter against the HaluEval dataset.
          </p>
            ...
            <div className="glass-card p-6">
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                <div className="p-3 rounded-lg bg-verified/10 border border-verified/30">
                  <div className="text-2xl font-bold text-verified">{results.confusionMatrix.truePositive + results.confusionMatrix.trueNegative}</div>
                  <div className="text-sm text-dark-muted">Correct Predictions</div>
                </div>
                <div className="p-3 rounded-lg bg-hallucination/10 border border-hallucination/30">
                  <div className="text-2xl font-bold text-hallucination">{results.confusionMatrix.falsePositive + results.confusionMatrix.falseNegative}</div>
                  <div className="text-sm text-dark-muted">Incorrect Predictions</div>
                </div>
              </div>
            </div>
              ...
              <div className="mt-4 space-y-2">
                {results.byCategory.map((cat) => (
                  <div key={cat.category} className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-dark-text">{cat.samples} samples</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {selectedTab === 'samples' && (
          ...
          >
            <div className="p-4 border-b border-dark-border">
              <h3 className="font-semibold text-dark-text">Sample Predictions</h3>
              <p className="text-sm text-dark-muted">Showing 5 of {results.totalSamples} samples</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-dark-bg/50">
                  <tr>
                    <th className="text-left p-4 text-sm font-medium text-dark-muted">ID</th>
                    <th className="text-left p-4 text-sm font-medium text-dark-muted">Response</th>
                    <th className="text-left p-4 text-sm font-medium text-dark-muted">Ground Truth</th>
                    <th className="text-left p-4 text-sm font-medium text-dark-muted">Prediction</th>
                    <th className="text-left p-4 text-sm font-medium text-dark-muted">Confidence</th>
                    <th className="text-left p-4 text-sm font-medium text-dark-muted">Result</th>
                  </tr>
                </thead>
                <tbody>
                  {results.samples.map((sample) => (
                    <tr key={sample.id} className="border-t border-dark-border hover:bg-dark-card/50">
                      <td className="p-4 text-sm text-dark-muted">#{sample.id}</td>
                      <td className="p-4 text-sm text-dark-text max-w-xs truncate">{sample.response}</td>
                      <td className="p-4">
                        <span className={`badge-${sample.groundTruth === 'hallucinated' ? 'hallucination' : 'verified'}`}>
                          {sample.groundTruth === 'hallucinated' ? 'Hallucinated' : 'Valid'}
                        </span>
                      </td>
                      <td className="p-4">
                        <span className={`badge-${sample.prediction === 'hallucinated' ? 'hallucination' : 'verified'}`}>
                          {sample.prediction === 'hallucinated' ? 'Hallucinated' : 'Valid'}
                        </span>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-dark-muted">{(sample.confidence * 100).toFixed(0)}%</span>
                        </div>
                      </td>
                      <td className="p-4">
                        {sample.isCorrect ? (
          ...
          >
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={performanceHistory}>
                  ...
                  <YAxis stroke="#94a3b8" domain={[0.6, 1]} tickFormatter={(v) => `${(v * 100).toFixed(0)}%`} />
                  <Tooltip
                    contentStyle={{
                      backgroundColor: '#1e293b',
                      border: '1px solid #334155',
                  ...
                  <Line type="monotone" dataKey="f1" stroke="#0ea5e9" strokeWidth={2} dot={{ fill: '#0ea5e9' }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <div className="mt-6 p-4 rounded-xl bg-verified/10 border border-verified/30">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-semibold text-dark-text">Version 2.0 Improvements</span>
              </div>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        <div className={`p-2 rounded-lg bg-dark-bg/50 ${color}`}>
          {icon}
        </div>
        {trend && (
          <span className={`text-xs ${trend.isPositive ? 'text-verified' : 'text-hallucination'}`}>
