...
import { ConfidenceMeter, StatusBadge, ClaimCard, StatCard } from '@/components/ui';
...
const demoSourceDocument: SourceDocument = {
  ...
  paragraphs: [
    { idx: 0, text: 'Type 2 diabetes mellitus is a chronic metabolic disorder characterized by hyperglycemia resulting from defects in insulin secretion, insulin action, or both. Early diagnosis and comprehensive management are essential for preventing complications.' },
    { idx: 1, text: 'The patient should be evaluated for diabetes complications at the time of diagnosis and annually thereafter. Key areas include retinopathy screening through dilated eye exam, nephropathy assessment via urine albumin-to-creatinine ratio and eGFR, neuropathy evaluation through comprehensive foot exam, and cardiovascular risk assessment.' },
    { idx: 2, text: 'HbA1c target should be individualized. For most non-pregnant adults, a reasonable HbA1c goal is <7.0%. A more stringent goal of <6.5% may be appropriate for selected individuals if achievable without significant hypoglycemia.' },
    { idx: 3, text: 'Metformin remains the first-line pharmacological agent unless contraindicated. It should be initiated at the time of diagnosis along with lifestyle modifications including medical nutrition therapy and physical activity.' },
    { idx: 4, text: 'GLP-1 receptor agonists and SGLT2 inhibitors have demonstrated cardiovascular and renal protective effects and should be considered for patients with established cardiovascular disease, heart failure, or chronic kidney disease.' },
    { idx: 5, text: 'Blood pressure targets for patients with diabetes should be <130/80 mmHg. First-line antihypertensive agents include ACE inhibitors or ARBs, especially for patients with albuminuria.' },
    { idx: 6, text: 'Statin therapy is recommended for all adults with diabetes aged 40-75 years. High-intensity statin therapy should be used for patients with established ASCVD.' },
    { idx: 7, text: 'All patients should receive diabetes self-management education and support (DSMES) at diagnosis and throughout their care.' },
  ],
};
...
const demoClaims: Claim[] = [
  {
    ...
    sourceSnippet: 'Type 2 diabetes mellitus is a chronic metabolic disorder characterized by hyperglycemia...',
  ...
  },
  {
    ...
    sourceSnippet: 'The patient should be evaluated for diabetes complications at the time of diagnosis and annually thereafter. Key areas include retinopathy screening...',
    explanation: 'Direct match with source paragraph 2 which explicitly states annual screening requirements.',
  },
  {
    id: 'claim_003',
    ...
    sourceSnippet: 'For most non-pregnant adults, a reasonable HbA1c goal is <7.0%.',
  ...
  },
  {
    ...
    sourceSnippet: 'Metformin remains the first-line pharmacological agent unless contraindicated.',
  ...
  },
  {
    ...
    sourceSnippet: 'Blood pressure targets for patients with diabetes should be <130/80 mmHg.',
    explanation: 'FACTUAL ERROR: The source document clearly states the blood pressure target as <130/80 mmHg, but the claim incorrectly states <140/90 mmHg. This is a significant discrepancy of 10 mmHg on both systolic and diastolic values.',
    correction: 'Blood pressure targets for patients with diabetes should be <130/80 mmHg, not <140/90 mmHg.',
    reasoning: 'The LLM appears to have confused the stricter diabetes-specific guidelines with general hypertension guidelines. This type of error could lead to inadequate blood pressure control in diabetic patients.',
  },
  {
    id: 'claim_006',
    ...
    sourceSnippet: 'GLP-1 receptor agonists and SGLT2 inhibitors have demonstrated cardiovascular and renal protective effects...',
    explanation: 'INCORRECT RECOMMENDATION: The source document explicitly recommends GLP-1 receptor agonists and SGLT2 inhibitors (NOT sulfonylureas) for patients with cardiovascular disease. The claim ignores important patient-specific factors.',
    ...
    reasoning: 'This is a potentially dangerous hallucination as sulfonylureas do not provide the cardiovascular benefits that GLP-1 RAs and SGLT2i offer. Following this incorrect advice could deny patients important cardioprotective benefits.',
  },
  {
    id: 'claim_007',
    ...
    sourceSnippet: 'Statin therapy is recommended for all adults with diabetes aged 40-75 years.',
...
export default function ResultsPage() {
  ...
  const handleClaimClick = (claim: Claim) => {
    ...
    if (claim.sourceParagraphIdx !== undefined) {
      ...
      setTimeout(() => {
        const element = document.getElementById(`para-${claim.sourceParagraphIdx}`);
  ...
  const tabs = [
    ...
    { id: 'corrections', label: 'Corrections', icon: Edit3, count: result.hallucinationCount },
    ...
    >
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        >
          ...
          {/* Tab Content */}
          <div className="p-4 max-h-[600px] overflow-y-auto" ref={sourceViewerRef}>
            <AnimatePresence mode="wait">
              ...
              {activeTab === 'corrections' && (
                <CorrectionsView
                  claims={result.claims.filter(c => c.status === 'hallucination')}
                />
              )}
      ...
      >
        ...
        <button onClick={() => router.push('/verify')} className="btn-secondary">
      ...
      <div className="space-y-3">
        {document.paragraphs.map((para) => (
          ...
          >
            ...
            <p className="text-sm text-dark-text leading-relaxed">{para.text}</p>
          </div>
        ))}
      </div>
    </motion.div>
  ...
  if (claims.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-dark-muted">No verified claims to show citations for.</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-sm text-dark-muted">({claims.length} verified claims)</span>
      </div>
          ...
          <div className="flex items-start gap-3 p-3 rounded-lg bg-dark-bg/50">
            ...
            <div>
              <p className="text-xs text-dark-muted mb-1">
                {sourceDoc.name} • Paragraph {(claim.sourceParagraphIdx || 0) + 1}
              </p>
              <p className="text-sm text-dark-muted italic">"{claim.sourceSnippet}"</p>
            </div>
          </div>
  ...
  if (claims.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-dark-text font-medium mb-2">No Corrections Needed!</p>
        <p className="text-dark-muted text-sm">All claims in this document are verified.</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-sm text-dark-muted">({claims.length} issues found)</span>
      </div>
        ...
        >
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-hallucination uppercase">Original (Hallucinated)</span>
            </div>
            <p className="text-sm text-dark-text line-through opacity-70">{claim.text}</p>
          </div>
          ...
          <div className="p-3 rounded-lg bg-verified/10 border border-verified/30">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-verified uppercase">Suggested Correction</span>
            </div>
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-dark-muted text-sm">
          Click on any claim in the left panel to see detailed explainability information.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="glass-card p-4 border-l-4 border-primary-500">
        ...
        <p className="text-sm text-dark-muted leading-relaxed">{claim.explanation}</p>
      </div>
      ...
      {claim.sourceSnippet && (
        <div className="glass-card p-4 border-l-4 border-verified">
          ...
          <div className="p-3 rounded-lg bg-dark-bg/50 border border-dark-border">
            ...
            <p className="text-sm text-dark-text italic">"{claim.sourceSnippet}"</p>
          </div>
        </div>
      )}
      ...
      <div className="glass-card p-4 border-l-4 border-unverified">
        ...
        <div className="space-y-3">
          <ConfidenceMeter confidence={claim.confidence} size="lg" showLabel={false} />
