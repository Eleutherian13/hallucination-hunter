...
export function StatCard({
  ...
  return (
    <motion.div whileHover={{ y: -2 }} className="glass-card-hover p-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          {trend && (
            <p
              className={`text-xs mt-2 ${trend.isPositive ? "text-verified" : "text-hallucination"}`}
            >
        ...
        {icon && <div className="p-3 rounded-xl bg-dark-bg/50">{icon}</div>}
      </div>
    </motion.div>
  );
}
...
export function LoadingSpinner({ size = "md", text }: LoadingSpinnerProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div
        className={`${sizeClasses[size]} border-dark-border border-t-primary-500 rounded-full animate-spin`}
      />
      ...
      {steps.map((step, idx) => (
        <div key={step.name} className="flex items-center">
          <div className="flex flex-col items-center">
            ...
            <p className="text-xs text-dark-muted mt-2 text-center w-20">
              {step.name}
            </p>
          </div>
          {idx < steps.length - 1 && (
            <div
              className={`w-16 h-0.5 mx-2 ${step.status === "completed" ? "bg-verified" : "bg-dark-border"}`}
            />
          )}
        </div>
      ))}
